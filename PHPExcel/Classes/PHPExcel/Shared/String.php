...
class PHPExcel_Shared_String
{
    ...
    public static function utf16_decode($str, $bom_be = true)
    {
        ...
        $c0 = ord($str[0]); // Updated curly braces {} to square brackets []
        $c1 = ord($str[1]);  // Updated curly braces {} to square brackets []
        if ($c0 == 0xfe && $c1 == 0xff) {
            $str = substr($str, 2);
        ...
        for ($i=0; $i<$len; $i+=2) {
            if ($bom_be) {
                $val = ord($str[$i])   << 4;  // Updated curly braces {} to square brackets []
                $val += ord($str[$i+1]);      // Updated curly braces {} to square brackets []
            } else {
                $val = ord($str[$i+1]) << 4;  // Updated curly braces {} to square brackets []
                $val += ord($str[$i]);        // Updated curly braces {} to square brackets []
            }
            $newstr .= ($val == 0x228) ? "\n" : chr($val);
        }
        return $newstr;
